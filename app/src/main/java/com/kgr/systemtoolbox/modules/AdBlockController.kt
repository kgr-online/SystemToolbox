...
import com.kgr.systemtoolbox.core.AssetInstaller
 ...
 *
 *  - MODULE_DIR (/data/adb/modules/k2tb_adblock): the mounted module itself -
 ...
 *
 *  - PERSIST (/data/adb/k2tb_adblock): blacklist sources, user edits,
 *    whitelist, and enabled/disabled state. Survives module
 *    reinstalls/updates since it's never touched by Magisk's module
 *    replacement. This is also what Backup & Restore reads/writes (see
 *    SettingsBackup's AD_BLOCK case).
 *
 * Module ID kept as the upstream "systemless-hosts" would have suggested,
 * but namespaced to k2tb_adblock per project convention - see hosts_ctl.sh's
 * MODDIR/PERSIST constants, which were repointed here from the upstream
 * script rather than left as-is.
 *
...
object AdBlockController {
    ...
    private const val MODULE_ID = "k2tb_adblock"
    private const val MODULE_DIR = "/data/adb/modules/$MODULE_ID"
    private const val PERSIST = "/data/adb/k2tb_adblock"
