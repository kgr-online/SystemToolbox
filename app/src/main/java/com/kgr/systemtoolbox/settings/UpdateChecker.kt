...
private const val TAG = "K2TB-Updater"
...
object UpdateChecker {
     ...
     *
     * This avoids the old bug where "4.5-beta2" vs "4.4-beta4" failed
     * numeric parsing (because "5-beta2" isn't an Int) and fell back to
    ...
    fun isNewer(latestTag: String, currentVersion: String): Boolean {
        ...
        // Numeric cores are equal (e.g. both "4.5") — fall back to suffix compare.
    ...
    /** Splits "4.5-beta2" into ("4.5", "beta2"); "4.5" into ("4.5", ""). */
    ...
    suspend fun downloadApk(
        ...
        val fileName = release.apkAssetName ?: "systemtoolbox-${release.tagName}.apk"
    ...
    suspend fun installApkAsRoot(apkFile: File): Boolean = withContext(Dispatchers.IO) {
        ...
        val result = Shell.cmd("pm install -r \"$path\"").exec()

        val output = result.out.joinToString("\n")
        Log.d(TAG, "pm install output: $output")

        if (!result.isSuccess) {
            Log.e(TAG, "Install failed: ${result.err.joinToString("\n")}")
        }
        result.isSuccess
    }
}
